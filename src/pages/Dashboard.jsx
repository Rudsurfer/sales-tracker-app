...
import { DAYS_OF_WEEK, TRANSACTION_TYPES } from "../constants";

const GoalProgressCard = ({ title, actual, target, percent }) => {
  const progress = Math.min(percent, 100);
  const strokeColor =
    progress >= 100 ? "#48BB78" : progress > 50 ? "#4299E1" : "#F56565";
  return (
...
export const Dashboard = ({
    ...
    const fetchData = async () => {
      ...
      try {
        const [salesRes, scheduleRes, stcRes, goalsRes] = await Promise.all([
          fetch(
            `${API_BASE_URL}/sales/${selectedStore}/${currentWeek}/${currentYear}`
          ).then((res) => res.json()),
          fetch(
            `${API_BASE_URL}/schedule/${selectedStore}/${currentWeek}/${currentYear}`
          ).then((res) => res.json()),
          fetch(
            `${API_BASE_URL}/stc/${selectedStore}/${currentWeek}/${currentYear}`
          ).then((res) => res.json()),
          fetch(
            `${API_BASE_URL}/goals/${selectedStore}/${currentWeek}/${currentYear}`
          ).then((res) => res.json()),
        ]);

        setSales(Array.isArray(salesRes) ? salesRes : []);
        setSchedule(
          scheduleRes?.status === "not_found" ? { rows: [] } : scheduleRes || { rows: [] }
        );
        setStcData(
          stcRes?.status === "not_found" ? { HourlyData: {} } : stcRes || { HourlyData: {} }
        );
  ...
  const {
    ...
    (sales || []).forEach((sale) => {
      if (sale.Type_ === TRANSACTION_TYPES.GIFT_CARD || sale.Type_ === TRANSACTION_TYPES.RETURN)
        return;
    ...
    const totalUnits = merchandiseSales.reduce(
      (sum, sale) =>
        sum +
        (sale.items || []).reduce(
    ...
    const totalTraffic = Object.values(stcData.HourlyData || {}).reduce(
      (daySum, dayData) =>
        daySum +
        Object.values(dayData || {}).reduce(
          (hourSum, hour) => hourSum + (Number(hour.traffic) || 0),
          0
        ),
      0
    );
    const totalSTCTransactions = Object.values(stcData.HourlyData || {}).reduce(
      (daySum, dayData) =>
        daySum +
        Object.values(dayData || {}).reduce(
          (hourSum, hour) => hourSum + (Number(hour.transactions) || 0),
          0
        ),
      0
    );
    ...
    (schedule.rows || []).forEach((row) => {
      ...
      (DAYS_OF_WEEK || []).forEach((dayName) => {
        const shift = row.shifts?.[dayName.toLowerCase()] || "";
        rowHours += parseShift(shift) || 0;
      });
      totalScheduledHours += rowHours;
      const emp = homeStoreEmployees.find((e) => e.EmployeeID === row.EmployeeID);
      const rate = getHourlyRate(emp);
      scheduledLaborCost += rowHours * rate;

      const actualHours = Object.values(row.actualHours || {}).reduce(
        (s, v) => s + (Number(v) || 0),
        0
      );
      totalActualHours += actualHours;
      actualLaborCost += actualHours * rate;
    });

    const weeklyGoal = Number(performanceGoals?.WeeklySalesTarget) || 0;
    const plannedPct =
      weeklyGoal > 0
        ? ((scheduledLaborCost + 0.02 * weeklyGoal) / weeklyGoal) * 100
        : 0;
    const actualPct =
      totalNetSales > 0
        ? ((actualLaborCost + 0.02 * totalNetSales) / totalNetSales) * 100
        : 0;

    const pctToColor = (p) =>
      p >= 20 ? "text-red-400" : p > 16 ? "text-yellow-400" : "text-green-400";

    return {
      netSales: totalNetSales,
      avgTransactionValue:
        totalTransactions > 0 ? totalNetSales / totalTransactions : 0,
      unitsPerTransaction:
        totalTransactions > 0 ? totalUnits / totalTransactions : 0,
      conversionRate:
        totalTraffic > 0 ? (totalSTCTransactions / totalTraffic) * 100 : 0,
  ...
  const dailyTarget =
    JSON.parse(performanceGoals.DailyGoals || "{}")?.[today.toLowerCase()] || 0;
  const dailyActual = (sales || [])
    .filter((s) => s.NameDay === today && s.Type_ !== TRANSACTION_TYPES.GIFT_CARD)
    .reduce((sum, s) => sum + (Number(s.TotalAmount) || 0), 0);
  const dailyPercent = dailyTarget > 0 ? (dailyActual / dailyTarget) * 100 : 0;
  const weeklyTarget = performanceGoals.WeeklySalesTarget || 0;
  const weeklyPercent =
    weeklyTarget > 0 ? (netSales / weeklyTarget) * 100 : 0;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        />
        <KPIStatCard
          title={t.dollarsPerTransaction}
          value={formatCurrency(avgTransactionValue)}
          icon={DollarSign}
          color="blue"
        />
        <KPIStatCard
          title={t.unitsPerTransaction}
          value={unitsPerTransaction.toFixed(2)}
          icon={Hash}
          color="orange"
        />
        <KPIStatCard
          title="Planned Wage Cost Percentage"
          value={`${plannedWagePercentage.toFixed(2)}%`}
          icon={Percent}
          color="purple"
          valueColorClass={plannedWageColor}
        />
        <KPIStatCard
          title="Actual Wage Cost Percentage"
          value={`${actualWagePercentage.toFixed(2)}%`}
          icon={Percent}
          color="purple"
          valueColorClass={actualWageColor}
        />
      </div>
