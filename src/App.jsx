                                            ...
                                            <div className="bg-gray-900 p-4 rounded-lg">
                                                ...
                                                <table className="w-full text-sm">
                                                    <thead className="text-xs text-gray-400 uppercase">
                                                        <tr><th className="px-4 py-2 text-left">{t.description}</th><th className="px-4 py-2 text-left">{t.category}</th><th className="px-4 py-2 text-right">{t.qty}</th><th className="px-4 py-2 text-right">{t.price}</th><th className="px-4 py-2 text-left">{t.soldBy}</th><th className="px-4 py-2 text-right">{t.subtotal}</th></tr>
                                                    </thead>
                                                    <tbody>
                                                        {(sale.items || []).map((item, idx) => (
                                                            <tr key={idx} className="border-b border-gray-700/50">
                                                                <td className="px-4 py-2">{item.description}</td><td className="px-4 py-2">{item.category}</td><td className="px-4 py-2 text-right">{item.quantity}</td><td className="px-4 py-2 text-right">{formatCurrency(item.price)}</td><td className="px-4 py-2 text-left">{item.salesRep}</td><td className="px-4 py-2 text-right">{formatCurrency(item.quantity * item.price)}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
...
const AddSaleForm = ({ scheduledEmployees, onAddSale, transactionType, t }) => {
    ...
    const [items, setItems] = useState([{ id: crypto.randomUUID(), description: '', category: SALE_CATEGORIES[0], quantity: 1, price: 0, salesRep: scheduledEmployees[0]?.name || '' }]);
    ...
    const handleAddItem = () => {
        setItems(current => [...current, { id: crypto.randomUUID(), description: '', category: SALE_CATEGORIES[0], quantity: 1, price: 0, salesRep: scheduledEmployees[0]?.name || '' }]);
    };
            ...
            <div className="space-y-2">
                ...
                {items.map((item, index) => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                        ...
                        <input type="number" placeholder={t.price} value={item.price} onChange={e => handleItemChange(item.id, 'price', Number(e.target.value))} className="col-span-2 bg-gray-800 border border-gray-600 rounded-md px-3 py-2" step="0.01" />
...
const AddReturnForm = ({ scheduledEmployees, onAddSale, t }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const finalItems = items.map(item => ({...item, quantity: -Math.abs(item.quantity)}));
            ...
            sales.forEach(sale => {
                ...
                (sale.items || []).forEach(item => {
                    if (item.salesRep === row.name) {
                        employeeSales += item.price * item.quantity;
                    }
                });
            });
...
const Reports = ({ sales, schedule, db, appId, selectedStore, currentYear, currentWeek, t }) => {
    ...
    const { totalSales, totalReturns, avgTransactionValue, unitsPerTransaction, giftCardSales } = useMemo(() => {
        const merchandiseSales = sales.filter(s => s.type !== TRANSACTION_TYPES.GIFT_CARD && s.type !== TRANSACTION_TYPES.RETURN);
